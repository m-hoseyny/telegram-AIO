...
aria2c --check-certificate=false --daemon=false --enable-rpc --rpc-listen-all --rpc-allow-origin-all=true \
   ...
   --max-overall-download-limit=30M --bt-tracker="[$tracker_list]" --disk-cache=32M \
   --max-overall-upload-limit=100K --max-concurrent-downloads=5 --summary-interval=0 \
   --peer-id-prefix=-qB4400- --user-agent=Wget/1.12 --peer-agent=qBittorrent/4.4.0 \
   --bt-enable-lpd=true --seed-time=0 --max-file-not-found=0 --max-tries=20 --file-allocation=none\
